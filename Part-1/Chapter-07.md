...
```bash
ping google.com               # Test connectivity/latency to a host
...
```bash
ssh-keygen -t rsa -b 4096 -C "ambika@example.com"
...
```bash
rsync -avz ./myproject/ ubuntu@54.210.15.22:/home/ubuntu/myproject/
...
|Feature |scp |rsync |
|---|---|---|
|Speed on repeat transfers |Slow (copies everything again) |Fast (copies only changes) |
|Resume interrupted transfer |No |Yes (-P flag) |
|Mirror/delete extra files |No |Yes (--delete) |
|Simplicity for one-off copy |Simpler |Slightly more flags |
|Best for |Quick single file copy |Backups, deployments, large directories |
...
1. Explain how SSH key-based authentication works, step by step.
2. What's the difference between scp and rsync?
3. Why would you use rsync -avz --delete, and what risk does it carry?
4. How do you connect to an EC2 instance using a .pem key file?
5. What does chmod 400 on a .pem file accomplish, and why is it required?
6. How would you test if a specific port is open on a remote server?
...
```bash
ssh -i key.pem user@ip                     # Connect via SSH
